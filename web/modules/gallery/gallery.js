...
const GALLERY_CONFIG = {
  ...
  categories: [
    { id: 'meizhaos', name: '美照',     folder: 'photos'  },
...
let MANIFEST          = {};  // folder -> 文件名数组，来自 manifest.json
...
function renderImages(folder) {
  const imgs  = MANIFEST[folder] ?? [];
  const grid  = document.getElementById('imageGrid');

  currentImages = imgs.map(f => `../../data/gallery/${folder}/${f}`);
  ...
  grid.innerHTML = imgs.map((f, i) => `
    <div class="image-card" data-index="${i}" tabindex="0" role="button" aria-label="查看第${i+1}张图片">
      <img src="../../data/gallery/${folder}/${f}" alt="${f}" loading="lazy">
